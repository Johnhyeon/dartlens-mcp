"""`--target auto` 가 ChatGPT 만 쓰는 PC에서 codex 를 고르는지 — DartLens.
...
def test_registered_command_prefers_uv_managed_binary(tmp_path):
    ...
    uv_exe = uv_bin / "dartlens.exe"
    uv_exe.write_text("", encoding="utf-8")
    pip_exe = tmp_path / "Scripts" / "dartlens.exe"
    ...
    with patch.object(setup_claude, "_uv_tool_bin_dirs", return_value=[uv_bin]), \
        ...
        assert setup_claude.resolve_server_entry("dartlens")["command"] == str(uv_exe)
