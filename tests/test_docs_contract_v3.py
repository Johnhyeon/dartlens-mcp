...
@pytest.mark.parametrize("key", DL_KEYS)
def test_readme_documents_v3_fields(key):
    assert key in _text("README.md"), f"README.md 에 {key} 설명이 없습니다."


def test_readme_declares_new_fields_optional():
    body = _text("README.md").lower()
    assert "v3" in body
