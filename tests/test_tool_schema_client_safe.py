...
dartlens/_tool_schema.py). 이 테스트는 우리가 내보내는 스키마와 인자 처리가
...
from dartlens._tool_schema import client_safe_input_schema, coerce_integer_strings
from dartlens.server import mcp
...
class CallThroughServerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def _call(self, arguments):
        ...
        from dartlens import _safe, server

        fetch = AsyncMock(return_value={"total_count": 0, "list": []})
        with patch.object(server, "_fetch_disclosure_list", fetch),              patch.object(_safe, "is_licensed", return_value=True):
            result = await mcp.call_tool("list_disclosures", arguments)
        blocks = result[0] if isinstance(result, tuple) else result
        text = "".join(getattr(b, "text", "") for b in blocks)
        return fetch.await_args, text

    async def test_numeric_yyyymmdd_is_accepted(self):
        """2026-09-17 새벽 실패한 호출 그대로 — bgn_de/end_de 가 숫자로 왔다."""
        call, text = await self._call({"bgn_de": 20260916, "end_de": 20260917,
                                       "kind": "material", "limit": 15})
        _corp, bgn, end, _kind, limit, _page = call.args
        self.assertEqual((bgn, end, limit), ("20260916", "20260917", 15))
        self.assertNotIn("valid string", text)

    async def test_all_optional_args_can_be_omitted(self):
        call, _ = await self._call({})
        corp, _bgn, _end, _kind, limit, page = call.args
        self.assertEqual((corp, limit, page), (None, 20, 1))
